<?php
    ...
    $('#add-delivery').on('click',function() {
      $('#personal-delivery-form').removeClass('hiddendiv');
    });
JS;
...
<div class="col s12 page-main valign-wrapper">
    <div class="content valign">
        <div class="page-wrap">
            <div class="row page-wrap-content cabinet">
                <img src="../images/close.png" alt="" class="img-close">
                <div class="col l12">
                    <ul class="row center-align nav-cabinet tabs">
                        <li class="tab col l3 m12 s12 black-text fs20 left-align"><a href="#contact-info">Контактная информация</a>
                        </li>
<!--                        <li class="tab col m12 s12 black-text hide-on-large-only left-align fs20"><a href="#change-password">Изменения-->
<!--                                пароля</a>-->
<!--                        </li>-->
                        <li class="tab col l3 m10 s12 black-text left-align fs20"><a href="#history-orders">История заказов</a></li>
                        ...
                        <li class="col l4 cabinet-form">
                            ...
                            <div class="row">
								...
								<?= $contactForm->field( $cabinet->model, 'birthday', [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
								                ...
								                ->label( 'Дата рождения', [ 'class' => 'active' ] ) ?>
                                <!-- У нас нет логина
                        ...
                        <li class="col l4 hide-on-med-and-down cabinet-form">
                            ...
                            <div class="row">
								<?= $changePasswordForm->field( $cabinet->passModel, 'user_id' )
								                       ...
								                       ->label( false ) ?>
								<?= $changePasswordForm->field( $cabinet->passModel, 'old_password',
								                                [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
								                       ->passwordInput( [ 'placeholder' => ' ' ] ) ?>
								<?= $changePasswordForm->field( $cabinet->passModel, 'password_repeat',
								                                [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
								                       ->passwordInput( [ 'placeholder' => ' ' ] ) ?>
								<?= $changePasswordForm->field( $cabinet->passModel, 'password',
								                                [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
								                       ->passwordInput( [ 'placeholder' => ' ' ] ) ?>
                            </div>
							<?php ActiveForm::end() ?>
                ...
                <div id="delivery-address">
                    ...
                    <div class="col l3 m5 s5 fs20">
						<?php if ( ! empty( $cabinet->model->deliveryData ) ): ?>
							<?php foreach ( $cabinet->model->deliveryData as $deliverIndex => $delivery ) : ?>
                                ...
                                <div class="hide">
									<?= $deliveryForm->field( $delivery, "[$deliverIndex]f_name",
									                          [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
									                 ->textInput( [ 'placeholder' => Yii::t( 'models/client', 'First name' ), ] )
									                 ->label( 'Имя получателя' ) ?>
									<?= $deliveryForm->field( $delivery, "[$deliverIndex]l_name",
									                          [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
									                 ->textInput( [ 'placeholder' => Yii::t( 'models/client', 'Last name' ), ] )
									                 ->label( 'Фамилия получателя' ) ?>
									<?= $deliveryForm->field( $delivery, "[$deliverIndex]city",
									                          [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
									                 ->textInput( [ 'placeholder' => Yii::t( 'models/client', 'City' ), ] )
									                 ->label( 'Город' ) ?>
									<?= $deliveryForm->field( $delivery, "[$deliverIndex]address",
									                          [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
									                 ->textInput( [ 'placeholder' => Yii::t( 'models/client', 'Address' ), ] )
									                 ->label( 'Адрес' ) ?>
									<?= $deliveryForm->field( $delivery, "[$deliverIndex]phone",
									                          [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
									                 ->textInput( [
										                              'class'       => 'input-phone',
                    ...
                    <div class="col l4 m5 cabinet-form">
                        <div class="row">
                            <div class="col s10">
                                <a href="#" class="fs20 fc-orange" id="add-delivery">Добавления нового адреса</a>
                            </div>
                        </div>
                        <div class="row hiddendiv" id="personal-delivery-form">
							<?php $deliverIndex = isset( $deliverIndex ) ? $deliverIndex + 1 : 0 ?>
							<?= $deliveryForm->field( $cabinet->deliveryModel, "[$deliverIndex]f_name",
							                          [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
							                 ->textInput( [ 'placeholder' => Yii::t( 'models/client', 'First name' ), ] )
							                 ->label( 'Имя получателя' ) ?>
							<?= $deliveryForm->field( $cabinet->deliveryModel, "[$deliverIndex]l_name",
							                          [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
							                 ->textInput( [ 'placeholder' => Yii::t( 'models/client', 'Last name' ), ] )
							                 ->label( 'Фамилия получателя' ) ?>
							<?= $deliveryForm->field( $cabinet->deliveryModel, "[$deliverIndex]city",
							                          [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
							                 ->textInput( [ 'placeholder' => Yii::t( 'models/client', 'City' ), ] )
							                 ->label( 'Город' ) ?>
							<?= $deliveryForm->field( $cabinet->deliveryModel, "[$deliverIndex]address",
							                          [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
							                 ->textInput( [ 'placeholder' => Yii::t( 'models/client', 'Address' ), ] )
							                 ->label( 'Адрес' ) ?>
							<?= $deliveryForm->field( $cabinet->deliveryModel, "[$deliverIndex]phone",
							                          [ 'options' => [ 'class' => 'col s10 input-field' ] ] )
							                 ->textInput( [
								                              'class'       => 'input-phone',
								                              'placeholder' => Yii::t( 'models/client', 'Phone' ),
