                ...
                <div class="panel-group" id="accordion">
                    <?php foreach($model->models as $index => $addressModel): ?>
                        <div class="panel panel-default">
                            <div class="panel-heading">
                                <p class="panel-title">
                                    <a data-toggle="collapse" data-parent="#accordion" href="#address-box-<?= $index ?>">
                                        <?php if(count($model->models) > 1 and !empty($addressModel->address)): ?>
                                            <?= ($addressModel->isGeneral) ? 'Основной адрес' : $addressModel->address ?>
                                        <?php else: ?>
                                            Добавить магазин
                                        <?php endif; ?>
                                    </a>
                            ...
                            <div id="address-box-<?= $index ?>" class="panel-collapse collapse <?= ($index == 0 and empty($addressModel->address)) ? 'in' : ''?>">
