<?php
	...
	class OrderForm extends Model{
		...
		public function init() {
			...
			//Gektor change !empty
			if ( empty( $del_data ) ) {
				$del_data = $this->client->client->deliveryData;
			}
			if ( is_array( $del_data ) ) {
				foreach ( $del_data as $index => $data ) {
					$this->deliveryData[ $index ] = new DeliveryAddressForm( $data );
				}
			} else {
				$this->deliveryData[] = new DeliveryAddressForm( $del_data );
			}
		...
		public function save() {
			...
			try{
				...
				if ( $isNewRecord ) {
					$sender = new Sender();
					if ( ! $sender->sendMail( $this->client->email, Yii::t( 'system/view', 'Test buy' ), 'mail-template-customer',
					                          [ 'model' => $this ] )
					) {
